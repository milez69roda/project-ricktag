...
var Midas = {
	...
	register: function( form ){ 
		...
		if ($("#card_yes:checked").val()) {
			...
			if( typeof this.base_url === 'undefined'){
				var modal = $('<div title="Warning"><p>Invalid Distributor</p></div>');					
	...
	contactus_action: function(form){
		data = $(form).serialize();
		var baseurl = this.base_url;
		$.ajax({
			type:"post",
			url:this.base_url+"/contactus_action",
			data:data,
			dataType:"json",
			success: function(xhr){
