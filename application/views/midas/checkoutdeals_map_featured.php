			...
			geocoder.geocode( { 'address': address}, function(results, status) {			
				if (status == google.maps.GeocoderStatus.OK) {
					...
					<?php 
						...
						//added the main feature stores to the sub stores
						if(  $featured_stores->main_store != '' ){
							$featured[] = $featured_stores->main_store;
						}
						...
						foreach ($featured as $storeid):
							...
							$islowed = true;
							if( $distributor_category == $store->category_id ){
								 
								if( !in_array($store->id, $exclusive_stores ) ){
									$islowed = false;
								}
							}
					...
					<?php $counter++; endif; endforeach; endif;?>		
				}
			});		 
